...
compone datos que un segundo cliente tendría que reimplementar (§4).
...
- **El proveedor `mock` no se usa en producción bajo ninguna circunstancia.** Un
  ...
  desarrollo.
- El valor correcto de `PROVEEDOR_VEHICULAR_ACTIVO` en producción es
  `consultas_ec`. Un nombre no registrado hace que `selector.py` caiga
  silenciosamente en `mock`: verificar siempre el catálogo de productos después
  de cambiarlo.
...
- **Referencias aportadas** (`publicaciones_referenciadas`): el usuario pega un link externo (FB/OLX/…); NO se raspa; entra `pendiente` y un admin la aprueba/rechaza (`/marketplace/referencias/...`). Desde M2.8 admite **contenido rico** (descripción, ciudad, kilometraje y hasta 5 `fotos` en JSONB, migración `0019`); editar ese contenido devuelve la referencia a `pendiente` (anti bait-and-switch). **Dos interacciones separadas (M2.9):** en el feed la tarjeta lleva al **detalle local** `/marketplace/referencias/{id}` (`GET /marketplace/referencias/{id}`, público, **solo aprobadas y activas** → 404 indistinto); desde ese detalle un **botón explícito** abre el anuncio original en pestaña nueva. La etiqueta **"Referencia externa · datos no verificados"** se muestra en ambos lugares. Regla de rutas: la dinámica `{referencia_id}` se declara AL FINAL del router para no capturar `mias`/`pendientes`.
