...
- **Marketplace**: publicaciones internas (light gratis / premium cobra tokens) con `GET /marketplace/feed`; referencias aportadas (link externo → moderación admin → enlace vivo, sin scraping); token de compra-venta (`enlaces_compartidos`, scope + expiración ≤7d).
